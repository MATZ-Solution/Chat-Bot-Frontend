...
export const sendMessage = async (messages, context = {}) => {
  try {
          ...
          const formattedResults = apiData.map((result, index) =>
            `
                  **${index + 1}:-  ${result.name}**
                  ...
                  ${isTopOrBestQuery ? `Total Reviews: ${result.scrapedAverageRating?.reviewer_count || ''}` : ''}
        ----------------
          `
          ).join('\n');
          const replyMessage = {
    ...
    const isTopOrBestQuery = /top|best|rated|ratings|rating|highest|high|highly/i.test(userMessageContent);
    ...
    if (collectionKeyword) {
      if (cityMatch && cityMatch[1] && stateMatch && stateMatch[1] && zipCodeMatch && zipCodeMatch[1]) {
        ...
        const apiUrl = `https://info-senior-bot-backend.vercel.app/api/${collectionKeyword}/search?city=${encodeURIComponent(city)}&state=${encodeURIComponent(state)}&zipCode=${encodeURIComponent(zipCode)}${isTopOrBestQuery ? '?top=true' : ''}`;
        ...
        const apiUrl = `https://info-senior-bot-backend.vercel.app/api/${collectionKeyword}/fullAddress?fullAddress=${encodeURIComponent(fullAddress)}${isTopOrBestQuery ? '?top=true' : ''}`;
        const { data, newMessages } = await fetchDataAndFormatResponse(apiUrl, collectionKeyword);
        return { data, messages: newMessages || [] };
      }
        ...
        const city = extractNameBeforeComma(cityMatch[1]);
        const state = extractNameBeforeComma(stateMatch[1]);
        const apiUrl = `https://info-senior-bot-backend.vercel.app/api/${collectionKeyword}/city_state?city=${encodeURIComponent(city)}&state=${encodeURIComponent(state)}${isTopOrBestQuery ? '?top=true' : ''}`;
        const { data, newMessages } = await fetchDataAndFormatResponse(apiUrl, collectionKeyword);
        return { data, messages: newMessages || [] };
      }
      // Check for city or zipCode matches
        ...
        const apiUrl = `https://info-senior-bot-backend.vercel.app/api/${collectionKeyword}/city/${encodeURIComponent(city)}${isTopOrBestQuery ? '?top=true' : ''}`;
        const { data, newMessages } = await fetchDataAndFormatResponse(apiUrl, collectionKeyword);
        return { data, messages: newMessages || [] };
      }
      else if (stateMatch && stateMatch[1]) {
        const state = stateMatch[1].trim();
        const apiUrl = `https://info-senior-bot-backend.vercel.app/api/${collectionKeyword}/state/${encodeURIComponent(state)}${isTopOrBestQuery ? '?top=true' : ''}`;
        const { data, newMessages } = await fetchDataAndFormatResponse(apiUrl, collectionKeyword);
        return { data, messages: newMessages || [] };
        ...
        const apiUrl = `https://info-senior-bot-backend.vercel.app/api/${collectionKeyword}/zipCode/${encodeURIComponent(zipCode)}${isTopOrBestQuery ? '?top=true' : ''}`;
        const { data, newMessages } = await fetchDataAndFormatResponse(apiUrl, collectionKeyword);
        return { data, messages: newMessages || [] };
      }
      else {
        const apiUrl = `https://info-senior-bot-backend.vercel.app/api/${collectionKeyword}${isTopOrBestQuery ? '?top=true' : ''}`;
        const { data, newMessages } = await fetchDataAndFormatResponse(apiUrl, collectionKeyword);
        return { data, messages: newMessages || [] };
      }
    }
      ...
      const promptMessages = Object.values(prompts).flatMap((prompt) => prompt.map((message) => ({ role: "model", parts: [message] })));
      const historyWithPrompts = [...promptMessages, ...messages.map((msg) => ({ role: msg.role, parts: [{ text: msg.parts }] }))];
